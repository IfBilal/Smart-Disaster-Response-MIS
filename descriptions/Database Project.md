...
```
 All the submissions must be made on Google classroom.
 You must submit files in a ZIP Folder named (24I-XXXX_24I-XXXX_24I-XXXX.zip).
The naming convention must be followed strictly.
 A group of up to 3 members is allowed; cross section groups are not permitted.
 Students are solely responsible for checking the final zip files for issues like corrupt files,
viruses in the file.
 Be prepared for viva or any follow up evaluation after Project submission.
 You are allowed to use AI tools only for frontend development.
 Zero marks will be awarded to the students involved in plagiarism.
 Late submission is not allowed.
```
## Smart Disaster Response MIS with Secure Transactions &

## Role-Based Control

A country frequently faces natural disasters such as floods, earthquakes, and urban fires. During
such events, thousands of emergency reports are generated in real time through mobile
applications, helplines, and monitoring systems. To efficiently manage crisis situations, the
government plans to develop a Smart Disaster Response Management Information System
(MIS). This system will be used by multiple stakeholders including emergency operators, field
officers, warehouse managers, finance officers, and administrators. The system must support real-
time coordination, high-volume transactions, secure data handling, analytical reporting,
database-level automation and performance optimization mechanisms, along through a
complete frontend interface, making it a full-fledged enterprise-level system.
...
```
Certain operations involve multiple dependent steps and must be treated as a single logical
unit (transaction).
Examples:
...
```
The system must incorporate database-level intelligent mechanisms to automate operations
...
```
o Faster response (optimized joins, pre-structured queries)
o Better security and abstraction
...
o Queries executed with indexing vs without indexing
 Measurement of:
o Query execution time
o Response latency
 Identification of:
o Scenarios where indexing improves performance
o Cases where indexing may introduce overhead (e.g., inserts/updates)
```
...
```
Students must now also justify:
